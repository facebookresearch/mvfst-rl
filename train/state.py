...
class Field(Enum):
    ...
    # SRTT = auto()
    # RTT_VAR = auto()
    NOISY_RTT_WITH_DELAY = auto()
    DELAY = auto()
    ACK_DELAY = auto()

    CWND = auto()
    THROUGHPUT_FROM_CWND = auto()
    # IN_FLIGHT = auto()
    # WRITABLE = auto()
    # SENT = auto()
    # RECEIVED = auto()
...
def get_from_state(flags, state, field, offset, dim=0):
    """
    ...
    if dim == 0:
        val = state[idx]  # straightforward indexing on first dimension
    else:
        idx_tensor = torch.tensor(idx).to(state.device)
        val = state.index_select(dim, idx_tensor).squeeze(dim)
    return val / flags.cc_env_obs_scaling


def get_sum(flags, state, field, dim=0):
    """Fetch the sum of `field` in `state`"""
    return get_from_state(flags, state, field, offset=OFFSET_SUM, dim=dim)


def get_mean(flags, state, field, dim=0):
    """Fetch the mean of `field` in `state`"""
    return get_from_state(flags, state, field, offset=OFFSET_MEAN, dim=dim)


def get_std(flags, state, field, dim=0):
    """Fetch the standard deviation of `field` in `state`"""
    return get_from_state(flags, state, field, offset=OFFSET_STD, dim=dim)


def get_min(flags, state, field, dim=0):
    """Fetch the minimum of `field` in `state`"""
    return get_from_state(flags, state, field, offset=OFFSET_MIN, dim=dim)


def get_max(flags, state, field, dim=0):
    """Fetch the maximum of `field` in `state`"""
    return get_from_state(flags, state, field, offset=OFFSET_MAX, dim=dim)
