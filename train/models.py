...
class SimpleNet(nn.Module):
    def __init__(
        ...
        # List of activation functions for task observation embedding layers.
        # If empty then there is no embedding: the task observation is used "as is".
        task_obs_embedding_activations: Iterable[str] = tuple(),
        # Size of the task observation embedding layer(s), if any.
        ...
        self.stochastic_evaluation = stochastic_evaluation
        ...
        self.task_obs_embedding_activations = self.get_activations(
            task_obs_embedding_activations
        )
        if self.task_obs_embedding_activations:
            self.task_obs_embedding_layers = nn.ModuleList(
                modules=[nn.Linear(task_obs_size, task_obs_embedding_size)]
                + [
                    nn.Linear(task_obs_embedding_size, task_obs_embedding_size)
                    for _ in range(len(self.task_obs_embedding_activations) - 1)
                ]
            )
        else:
            self.task_obs_embedding_layers = []
            task_obs_embedding_size = task_obs_size
        ...
        self.fc_layers = nn.ModuleList(
            modules=[
                nn.Linear(
                    input_size
                    + task_obs_embedding_size * self.use_task_obs_in_network_input,
                    hidden_size,
                )
            ]
            + [nn.Linear(hidden_size, hidden_size) for _ in range(len(activations) - 1)]
        )
        self.activations = self.get_activations(activations)
        ...
        if self.actor_head:
            self.policy = nn.Linear(
                core_output_size
                + task_obs_embedding_size * self.use_task_obs_in_actor_head,
                self.num_actions,
            )
        ...
        if self.critic_head:
            self.baseline = nn.Linear(
                core_output_size
                + task_obs_embedding_size * self.use_task_obs_in_critic_head,
                1,
            )
    ...
    def concat_to_task_obs(self, tensor, task_obs):
        ...
        for layer, activation in zip(
            self.task_obs_embedding_layers, self.task_obs_embedding_activations
        ):
            task_obs = layer(task_obs)
            if activation is not None:
                task_obs = activation(task_obs)
    ...
    def get_activations(self, activations):
        ...
        for act in activations:
            if act == "linear":
                act_functions.append(None)
            elif act == "relu":
                act_functions.append(F.relu)
            elif act == "tanh":
                act_functions.append(torch.tanh)
            else:
    ...
    def forward(self, last_actions, env_outputs, task_obs, core_state, unroll=False):
        ...
        if self.actor_head:
            ...
            if self.training or self.stochastic_evaluation:
                action = torch.multinomial(
                    F.softmax(policy_logits, dim=1), num_samples=1
                )
            else:
                # Don't sample when testing (unless `stochastic_evaluation` is True).
