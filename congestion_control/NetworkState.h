...
struct NetworkState {
public:
  // NOTE: If fields are added, make sure to also update:
  //   - fieldToString() in NetworkState.cpp
  //   - the corresponding `Field` enum in state.py
  ...
  static torch::Tensor
  toTensor(const quic::utils::vector<NetworkState> &states);
  static void toTensor(const quic::utils::vector<NetworkState> &states,
                       torch::Tensor &tensor);

  static quic::utils::vector<NetworkState>
  fromTensor(const torch::Tensor &tensor);

  static std::string fieldToString(const uint16_t field);
  static std::string fieldToString(const Field field);

private:
  quic::utils::vector<float> data_;
};
